...
chrome.runtime.onMessage.addListener(
    function ( request, sender, sendResponse ) {
        console.log( request );
        init( request.guide, request.ignored );
        sendResponse( {
            status: 'running'
        } );
    }
...
function init( guide, ignored ) {
    var t = document.querySelector( '.audit-push-results' );
    t.removeAttribute( 'style' );
    t.firstChild.removeAttribute( 'style' );
    parseStyleSheets();
    document.addEventListener( 'styleSheetsParsed', function () {
        auditStyleGuide( guide, ignored, null );
    } );
}
...
//     prismJS.src = 'https://cdnjs.cloudflare.com/ajax/libs/prism/1.6.0/prism.min.js';
            ...
            ( function ( index ) {
                getStyleSheetRules( sheet, function ( rules, href ) {
                    ...
                    if ( ++count === sheetLength ) {
                        ...
                        var event = new CustomEvent( 'styleSheetsParsed', {
                            count: count
                        } );
